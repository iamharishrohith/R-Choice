...
// Create a generic transporter using SMTP
// To prevent crashes during development if variables are missing, we default to dummy variables 
// but they won't actually send emails unless configured properly.
...
export async function sendCompanyResultEmail(
  ...
  try {
    ...
    // Only attempt actual send if SMTP is configured to prevent Next.js app crashes locally
    if (SMTP_USER && SMTP_PASS) {
      ...
      console.log(`[Mail Generator] Verification email sent to ${toEmail}`);
    } else {
      console.log(`[Mail Generator - DEV MODE] Email intended for ${toEmail} with code ${verificationCode}`);
    }
    return { success: true };
  } catch (error) {
    console.error("[Mail Generator Error]:", error);
